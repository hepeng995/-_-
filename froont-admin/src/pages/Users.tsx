        ...
        <div className="flex flex-wrap items-center gap-6 mb-6">
          <div className="flex items-center gap-2">
            ...
            <input type="text" placeholder="请输入用户名" value={filterUsername}
              ...
              className="border border-gray-300 rounded px-3 py-1.5 text-sm w-48 focus:outline-none focus:border-[#409eff] transition-colors" />
          </div>
          <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)}
              className="border border-gray-300 rounded px-3 py-1.5 text-sm w-32 focus:outline-none focus:border-[#409eff] bg-white transition-colors">
              <option value=""></option>
          ...
          <div className="flex items-center gap-2 ml-2">
            <button onClick={handleSearch} className="bg-[#409eff] hover:bg-[#66b1ff] text-white px-4 py-1.5 rounded text-sm transition-colors">查询</button>
            <button onClick={handleResetFilters} className="bg-white border border-gray-300 hover:text-[#409eff] hover:border-[#409eff] text-gray-600 px-4 py-1.5 rounded text-sm transition-colors">重置</button>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={handleOpenAdd} className="bg-[#409eff] hover:bg-[#66b1ff] text-white px-4 py-1.5 rounded text-sm transition-colors">添加用户</button>
          <button onClick={handleBatchDelete} disabled={selectedIds.length === 0}
            className="bg-[#f56c6c] hover:bg-[#f78989] text-white px-4 py-1.5 rounded text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed">批量删除</button>
          <button onClick={() => handleBatchStatus(false)} disabled={selectedIds.length === 0}
            className="bg-[#e6a23c] hover:bg-[#ebb563] text-white px-4 py-1.5 rounded text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed">批量禁用</button>
          <button onClick={() => handleBatchStatus(true)} disabled={selectedIds.length === 0}
            className="bg-[#67c23a] hover:bg-[#85ce61] text-white px-4 py-1.5 rounded text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed">批量启用</button>
        </div>
      </Card>
      ...
      <Card className="p-0 overflow-hidden">
          ...
          <table className="w-full text-left border-collapse border border-gray-300">
            <thead>
              <tr className="bg-[#f8f8f9] text-[#909399] text-sm font-bold">
                ...
                <th style={{ width: columnWidths.actions }} className="py-3 px-4 text-center sticky right-0 bg-[#f8f8f9] z-20 shadow-[-2px_0_5px_rgba(0,0,0,0.05)] border border-gray-300">操作</th>
              </tr>
            </thead>
            <tbody className="text-sm text-[#606266]">
              {users.length > 0 ? users.map((user) => (
                <tr key={user.id} className="hover:bg-[#f5f7fa] transition-colors">
                  ...
                  <td className="py-3 px-4 text-center border border-gray-300">
                    <span className="text-[#67c23a] bg-[#f0f9eb] border border-[#e1f3d8] px-2 py-0.5 rounded text-xs whitespace-nowrap">
                      {ROLE_MAP[user.role] || user.role}
                    </span>
                  </td>
                  <td className="py-3 px-4 text-center border border-gray-300">
                    <span className={`px-2 py-0.5 rounded text-xs border whitespace-nowrap ${
                      user.enabled ? 'text-[#67c23a] bg-[#f0f9eb] border-[#e1f3d8]' : 'text-[#f56c6c] bg-[#fef0f0] border-[#fde2e2]'
                    }`}>
                      {user.enabled ? '启用' : '禁用'}
                    </span>
                  </td>
                  <td className="py-3 px-4 text-center border border-gray-300 truncate" title={user.createdAt}>{user.createdAt}</td>
                  <td className="py-3 px-4 text-center sticky right-0 bg-white group-hover:bg-[#f5f7fa] transition-colors z-10 shadow-[-2px_0_5px_rgba(0,0,0,0.02)] border border-gray-300">
                    <div className="flex items-center justify-center gap-2 flex-wrap w-max mx-auto">
                      <button onClick={() => handleOpenEdit(user)} className="bg-[#409eff] hover:bg-[#66b1ff] text-white px-3 py-1 rounded text-xs transition-colors whitespace-nowrap">编辑</button>
                      <button onClick={() => toggleUserStatus(user)} className="bg-[#e6a23c] hover:bg-[#ebb563] text-white px-3 py-1 rounded text-xs transition-colors whitespace-nowrap">
                        {user.enabled ? '禁用' : '启用'}
                      </button>
                      <button onClick={() => handleResetPassword(user)} className="bg-[#909399] hover:bg-[#a6a9ad] text-white px-3 py-1 rounded text-xs transition-colors whitespace-nowrap">重置密码</button>
                      <button onClick={() => confirmDelete(user.id)} className="bg-[#f56c6c] hover:bg-[#f78989] text-white px-3 py-1 rounded text-xs transition-colors whitespace-nowrap">删除</button>
                    </div>
                  </td>
                </tr>
              )) : (
        ...
        <div className="p-4 border-t border-gray-100 flex items-center text-sm text-[#606266]">
          ...
          <select value={pageSize} onChange={(e) => { setPageSize(Number(e.target.value)); setCurrent(1); }}
            className="border border-gray-300 rounded px-2 py-1 mr-4 focus:outline-none focus:border-[#409eff] bg-white transition-colors">
          ...
          <div className="flex items-center gap-2 mr-4">
            <button onClick={() => setCurrent(Math.max(1, current - 1))} disabled={current <= 1}
              className="text-gray-400 hover:text-[#409eff] disabled:opacity-50 transition-colors">&lt;</button>
            {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
              const page = i + 1;
              return (
                <button key={page} onClick={() => setCurrent(page)}
                  className={`${page === current ? 'text-[#409eff] font-bold' : 'text-gray-400 hover:text-[#409eff]'}`}>{page}</button>
              );
            })}
          ...
          <div className="flex items-center gap-2">
            ...
            <input type="number" min={1} max={totalPages} value={current}
              ...
              className="border border-gray-300 rounded w-12 text-center py-1 focus:outline-none focus:border-[#409eff] transition-colors" />
            <span>页</span>
          </div>
        </div>
      </Card>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input type="text" value={formData.username} onChange={(e) => setFormData({...formData, username: e.target.value})}
              className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-[#409eff]" placeholder="输入用户名" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">真实姓名</label>
            <input type="text" value={formData.realName} onChange={(e) => setFormData({...formData, realName: e.target.value})}
              className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-[#409eff]" placeholder="输入真实姓名" />
          </div>
          {modalMode === 'add' && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">密码</label>
              <input type="password" value={formData.password} onChange={(e) => setFormData({...formData, password: e.target.value})}
                className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-[#409eff]" placeholder="输入密码" />
            </div>
          )}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">电子邮箱</label>
            <input type="email" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})}
              className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-[#409eff]" placeholder="输入邮箱" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">手机号</label>
            <input type="text" value={formData.phoneNumber} onChange={(e) => setFormData({...formData, phoneNumber: e.target.value})}
              className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-[#409eff]" placeholder="输入手机号" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">角色</label>
            <select value={formData.role} onChange={(e) => setFormData({...formData, role: e.target.value})}
              className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-[#409eff] bg-white">
          ...
          <div className="pt-4 flex justify-end gap-3">
            <button onClick={() => setIsModalOpen(false)} className="px-4 py-2 text-sm font-medium text-gray-600 bg-white border border-gray-300 rounded hover:text-[#409eff] hover:border-[#409eff]">取消</button>
            <button onClick={handleSave} disabled={saving} className="px-4 py-2 text-sm font-medium text-white bg-[#409eff] rounded hover:bg-[#66b1ff] disabled:opacity-50">
              {saving ? '保存中...' : '确定'}
            </button>
          </div>
        </div>
      ...
      <Modal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} title="提示">
        <div className="flex flex-col">
          <div className="flex items-center gap-3 mb-6">
            <AlertCircle className="text-[#e6a23c]" size={24} />
            <p className="text-gray-700">此操作将永久删除该用户, 是否继续?</p>
          </div>
          <div className="flex justify-end gap-3">
            <button onClick={() => setIsDeleteModalOpen(false)} className="px-4 py-1.5 text-sm font-medium text-gray-600 bg-white border border-gray-300 rounded hover:text-[#409eff]">取消</button>
            <button onClick={handleDelete} className="px-4 py-1.5 text-sm font-medium text-white bg-[#409eff] rounded hover:bg-[#66b1ff]">确定</button>
          </div>
        </div>
      </Modal>
    </div>
