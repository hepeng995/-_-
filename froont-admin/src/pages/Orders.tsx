...
export default function Orders() {
  ...
  const getActions = (order: Order) => {
    ...
    if (order.orderStatus === 1) actions.push({ label: '取消', color: 'bg-[#f56c6c]', action: () => handleCancel(order.id) });
    if (order.orderStatus === 2) actions.push({ label: '发货', color: 'bg-[#409eff]', action: () => handleUpdateStatus(order.id, 3) });
    if (order.orderStatus === 3) actions.push({ label: '确认收货', color: 'bg-[#67c23a]', action: () => handleUpdateStatus(order.id, 4) });
    return actions;
  };
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6">
        <div className="flex flex-wrap items-center gap-6 mb-6">
          <div className="flex items-center gap-2"><label className="text-sm text-gray-600">订单号</label><input type="text" value={filterOrderNo} onChange={(e) => setFilterOrderNo(e.target.value)} placeholder="搜索订单号" className="border border-gray-300 rounded px-3 py-1.5 text-sm w-48 focus:outline-none focus:border-[#409eff]" /></div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse border border-gray-300">
            ...
            <tbody className="text-sm text-[#606266]">
              {orders.map(order => (
                <tr key={order.id} className="hover:bg-[#f5f7fa]">
                  ...
                  <td className="py-3 px-4 text-center border border-gray-300"><span className={`px-2 py-0.5 rounded text-xs ${order.paymentStatus === 1 ? 'text-[#67c23a] bg-[#f0f9eb]' : 'text-[#e6a23c] bg-[#fdf6ec]'}`}>{PAYMENT_STATUS_MAP[order.paymentStatus] || '-'}</span></td>
                  <td className="py-3 px-4 text-center border border-gray-300"><span className="px-2 py-0.5 rounded text-xs border text-[#409eff] bg-[#ecf5ff] border-[#d9ecff]">{ORDER_STATUS_MAP[order.orderStatus] || '-'}</span></td>
                  <td className="py-3 px-4 text-center border border-gray-300">{order.createdAt}</td>
                  <td className="py-3 px-4 text-center border border-gray-300">
                    <div className="flex items-center justify-center gap-1 flex-wrap">
                      <button onClick={() => handleViewDetail(order)} className="bg-[#409eff] text-white px-2 py-1 rounded text-xs">详情</button>
              ...
              {orders.length === 0 && !loading && <tr><td colSpan={8} className="py-12 text-center text-gray-500 border border-gray-300">暂无数据</td></tr>}
            </tbody>
          </table>
        </div>
          ...
          <div className="bg-white rounded-lg shadow-xl w-[700px] max-w-[90vw] max-h-[85vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div><span className="text-gray-500">实付金额：</span><span className="text-[#f56c6c] font-bold">¥{detailOrder.actualAmount}</span></div>
