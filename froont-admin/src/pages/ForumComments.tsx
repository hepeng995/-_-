    ...
    <div className="space-y-6">
      ...
      <Card className="p-6">
        <div className="flex flex-wrap items-center gap-6 mb-6">
          <div className="flex items-center gap-2"><label className="text-sm text-gray-600">用户名</label><input type="text" value={filterUsername} onChange={(e) => setFilterUsername(e.target.value)} placeholder="搜索" className="border border-gray-300 rounded px-3 py-1.5 text-sm w-48 focus:outline-none focus:border-[#409eff]" /></div>
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => handleBatchAudit(1)} disabled={selectedIds.length === 0} className="bg-[#67c23a] text-white px-4 py-1.5 rounded text-sm disabled:opacity-50">批量通过</button>
          <button onClick={() => handleBatchAudit(2)} disabled={selectedIds.length === 0} className="bg-[#e6a23c] text-white px-4 py-1.5 rounded text-sm disabled:opacity-50">批量拒绝</button>
          <button onClick={handleBatchDelete} disabled={selectedIds.length === 0} className="bg-[#f56c6c] text-white px-4 py-1.5 rounded text-sm disabled:opacity-50">批量删除</button>
        </div>
      </Card>
          ...
          <table className="w-full text-left border-collapse border border-gray-300">
            ...
            <tbody className="text-sm text-[#606266]">
              {comments.map(comment => (
                <tr key={comment.id} className="hover:bg-[#f5f7fa]">
                  ...
                  <td className="py-3 px-4 text-center border border-gray-300">
                    <span className={`px-2 py-0.5 rounded text-xs border ${comment.status === 1 ? 'text-[#67c23a] bg-[#f0f9eb] border-[#e1f3d8]' : comment.status === 0 ? 'text-[#e6a23c] bg-[#fdf6ec] border-[#faecd8]' : 'text-[#f56c6c] bg-[#fef0f0] border-[#fde2e2]'}`}>{forumApi.getStatusName(comment.status)}</span>
                  </td>
                  <td className="py-3 px-4 text-center border border-gray-300">{comment.createdAt}</td>
                  <td className="py-3 px-4 text-center border border-gray-300">
                    <div className="flex items-center justify-center gap-1">
                      {comment.status === 0 && <button onClick={() => handleAuditClick(comment)} className="bg-[#409eff] text-white px-2 py-1 rounded text-xs">审核</button>}
                      <button onClick={() => handleDelete(comment.id)} className="bg-[#f56c6c] text-white px-2 py-1 rounded text-xs">删除</button>
                    </div>
                  </td>
                </tr>
              ))}
