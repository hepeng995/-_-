    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm flex items-center justify-between">
        ...
        <div className="flex gap-3">
          <button onClick={() => setIsAddModalOpen(true)} className="px-4 py-2 bg-bamboo-500 hover:bg-bamboo-400 text-white rounded text-sm transition-colors">新增配置</button>
          <button onClick={handleRefreshCache} className="px-4 py-2 bg-sprout-500 hover:bg-sprout-400 text-white rounded text-sm transition-colors">刷新缓存</button>
        </div>
      </div>
        ...
        <div className="flex border-b border-gray-200 px-6 pt-4">
          {groups.map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`px-6 py-3 text-sm font-medium transition-colors relative ${activeTab === tab ? 'text-bamboo-500' : 'text-gray-600 hover:text-gray-800'}`}>
              {tab}
          ...
          <div className="bg-white rounded-lg shadow-xl w-[600px] max-w-[90vw] flex flex-col">
            ...
            <div className="p-6 space-y-4">
              <div className="flex items-center">
                ...
                <input type="text" value={editForm.configName} onChange={(e) => setEditForm({...editForm, configName: e.target.value})} className="flex-1 border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-bamboo-500/40 focus-visible:border-bamboo-500" />
              </div>
              <div className="flex items-center">
              ...
              <div className="flex items-center">
                ...
                <input type="text" value={editForm.configValue} onChange={(e) => setEditForm({...editForm, configValue: e.target.value})} className="flex-1 border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-bamboo-500/40 focus-visible:border-bamboo-500" />
              </div>
              <div className="flex items-center">
                <label className="w-24 text-right mr-4 text-gray-700 text-sm">参数类型</label>
                <select value={editForm.configType} onChange={(e) => setEditForm({...editForm, configType: e.target.value})} className="flex-1 border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-bamboo-500/40 focus-visible:border-bamboo-500 bg-white">
              ...
              <div className="flex items-center">
                ...
                <input type="text" value={editForm.groupName} onChange={(e) => setEditForm({...editForm, groupName: e.target.value})} className="flex-1 border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-bamboo-500/40 focus-visible:border-bamboo-500" />
              </div>
              <div className="flex items-center">
                <label className="w-24 text-right mr-4 text-gray-700 text-sm">排序</label>
                <input type="number" value={editForm.sort} onChange={(e) => setEditForm({...editForm, sort: Number(e.target.value)})} className="flex-1 border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-bamboo-500/40 focus-visible:border-bamboo-500" />
              </div>
              <div className="flex items-start">
                <label className="w-24 text-right mr-4 mt-2 text-gray-700 text-sm">备注说明</label>
                <textarea value={editForm.remark} onChange={(e) => setEditForm({...editForm, remark: e.target.value})} rows={3} className="flex-1 border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-bamboo-500/40 focus-visible:border-bamboo-500 resize-y"></textarea>
              </div>
            </div>
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-100">
              ...
              <button onClick={handleEditSave} disabled={saving} className="px-5 py-2 bg-bamboo-500 hover:bg-bamboo-400 text-white rounded text-sm disabled:opacity-50">{saving ? '保存中...' : '确定'}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-lg shadow-xl w-[600px] max-w-[90vw] flex flex-col">
            ...
            <div className="p-6 space-y-4">
              <div className="flex items-center">
                <label className="w-24 text-right mr-4 text-gray-700 text-sm"><span className="text-red-500 mr-1">*</span>参数名称</label>
                <input type="text" value={addForm.configName} onChange={(e) => setAddForm({...addForm, configName: e.target.value})} className="flex-1 border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-bamboo-500/40 focus-visible:border-bamboo-500" />
              </div>
              <div className="flex items-center">
                <label className="w-24 text-right mr-4 text-gray-700 text-sm"><span className="text-red-500 mr-1">*</span>参数键名</label>
                <input type="text" value={addForm.configKey} onChange={(e) => setAddForm({...addForm, configKey: e.target.value})} className="flex-1 border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-bamboo-500/40 focus-visible:border-bamboo-500" />
              </div>
              <div className="flex items-center">
                <label className="w-24 text-right mr-4 text-gray-700 text-sm"><span className="text-red-500 mr-1">*</span>参数值</label>
                <input type="text" value={addForm.configValue} onChange={(e) => setAddForm({...addForm, configValue: e.target.value})} className="flex-1 border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-bamboo-500/40 focus-visible:border-bamboo-500" />
              </div>
              <div className="flex items-center">
                <label className="w-24 text-right mr-4 text-gray-700 text-sm">参数类型</label>
                <select value={addForm.configType} onChange={(e) => setAddForm({...addForm, configType: e.target.value})} className="flex-1 border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-bamboo-500/40 focus-visible:border-bamboo-500 bg-white">
                  <option value="text">文本</option><option value="number">数字</option><option value="boolean">布尔</option><option value="select">选择</option><option value="date">日期</option>
                </select>
              </div>
              <div className="flex items-center">
                <label className="w-24 text-right mr-4 text-gray-700 text-sm"><span className="text-red-500 mr-1">*</span>分组名称</label>
                <input type="text" value={addForm.groupName} onChange={(e) => setAddForm({...addForm, groupName: e.target.value})} className="flex-1 border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-bamboo-500/40 focus-visible:border-bamboo-500" />
              </div>
              <div className="flex items-center">
                <label className="w-24 text-right mr-4 text-gray-700 text-sm">排序</label>
                <input type="number" value={addForm.sort} onChange={(e) => setAddForm({...addForm, sort: Number(e.target.value)})} className="flex-1 border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-bamboo-500/40 focus-visible:border-bamboo-500" />
              </div>
              <div className="flex items-start">
                <label className="w-24 text-right mr-4 mt-2 text-gray-700 text-sm">备注说明</label>
                <textarea value={addForm.remark} onChange={(e) => setAddForm({...addForm, remark: e.target.value})} rows={3} className="flex-1 border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-bamboo-500/40 focus-visible:border-bamboo-500 resize-y"></textarea>
              </div>
            </div>
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-100">
              ...
              <button onClick={handleAdd} disabled={saving} className="px-5 py-2 bg-bamboo-500 hover:bg-bamboo-400 text-white rounded text-sm disabled:opacity-50">{saving ? '新增中...' : '确定'}</button>
            </div>
          </div>
        </div>
      )}
