...
/** 批量删除景点 */
export function batchDeleteAttractions(ids: number[]) {
  return request.delete<void>('/attractions/batch', { data: ids });
}

/** 批量更新景点状态 */
export function batchUpdateAttractionStatus(ids: number[], status: number) {
  return request.put<void>('/attractions/batch/status', { ids, status });
}
...
export default {
  ...
  batchDeleteAttractions, batchUpdateAttractionStatus,
};
