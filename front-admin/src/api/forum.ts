...
/** 批量审核帖子 */
export function batchAuditPosts(ids: number[], status: number) {
  return request.post<void>('/forum/posts/batch-audit', { ids, status });
}

/** 批量删除帖子 */
export function batchDeletePosts(ids: number[]) {
  return request.delete<void>('/forum/posts/batch', { data: ids });
}
...
export default {
  ...
  batchAuditPosts, batchDeletePosts,
};
